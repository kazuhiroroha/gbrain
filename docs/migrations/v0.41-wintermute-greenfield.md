# Migrating wintermute brain to gbrain v0.41 (greenfield)

The v0.41 lens packs ship a one-shot importer that re-ingests your
existing wintermute brain (`~/git/brain/atoms/`, `concepts/`, `ideas/`)
through the new ingestion cathedral. Pages land in gbrain with an
`imported_from: wintermute-greenfield` frontmatter marker so the new
...
```bash
gbrain capture --source wintermute-greenfield \
  --repo ~/git/brain \
  --dry-run \
  --limit 100
```
...
- `skipped_invalid` — files that failed validation (these append to
  `~/.gbrain/audit/wintermute-greenfield-failures-YYYY-Www.jsonl`)

Inspect the audit JSONL:
```bash
ls ~/.gbrain/audit/wintermute-greenfield-failures-*.jsonl
cat ~/.gbrain/audit/wintermute-greenfield-failures-*.jsonl | jq .
```

Common failures:
- **Empty frontmatter** — file has `---` but no fields. Not a real
  brain page; safe to leave skipped.
- **Malformed YAML** — fix the file in wintermute then re-run.
- **Missing required field** — usually means the original wintermute
...
1. Walks `atoms/{YYYY-MM-DD}/*.md`, `concepts/*.md`, `ideas/*.md` in
   ...
   where they left off.
2. Stamps `imported_from: wintermute-greenfield` + `imported_at:
...
## Retiring wintermute's parallel crons

After verifying the import, retire wintermute's parallel atom
pipeline cron entries. In `~/git/wintermute/workspace/cron.json`:
...
The wintermute skills themselves shrink to thin wrappers:
...
failures in wintermute and want to retry just those:
